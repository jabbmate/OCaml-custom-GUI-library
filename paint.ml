...
type shape = 
  | Line of {color: Gctx.color; thickness: int; p1: point; p2: point}
...
let repaint (g: gctx) : unit =
  let draw_shape (s: shape) : unit =
    begin match s with
      ...
      | Ellipse e -> 
        draw_ellipse (with_params g e.color e.thickness) e.center e.rx e.ry
    end in
...
let paint_action (gc:gctx) (event:event) : unit =
  ...
  begin match (event_type event) with
    ...
    | MouseDrag ->
      ...
      (begin match paint.mode with 
          ...
          | LineEndMode last ->
            ...
            thickness=paint.thickness; p1=last; p2=p})
          | PointMode ->
            let points_list =
              begin match paint.preview with
          ...
          | EllipseStartMode -> ()
          | EllipseEndMode prev_pos -> 
            let (x1, y1) = prev_pos in 
            let (x2, y2) = p in
            let rad_x = (Int.abs (x2 - x1) / 2) in
            let rad_y = (Int.abs (y2 - y1) / 2) in
            let c = (rad_x + x1, rad_y + y1) in
            paint.preview <-
            Some (Ellipse {color=paint.color; 
            thickness=paint.thickness; center=c; rx=rad_x; ry=rad_y})
    ...
    | MouseUp ->
      ...
      (begin match paint.mode with 
          | LineStartMode -> ()
          | LineEndMode last_pos ->
            Deque.insert_tail
              (Line {color=paint.color; 
              thickness=paint.thickness; p1=last_pos; p2=p}) paint.shapes;
...
(** This function runs when the Line button is clicked. 
    It simple sets the mode to LineStartMode. *)
...
(** This function runs when the Point button is clicked. 
    It simply sets the mode to PointMode. *)
...
(** This function runs when the Ellipse button is clicked. 
    It simply sets the mode to EllipseStartMode. *)
...
let checkbox_thickness () : widget = 
  let (widget, val_control) = checkbox false "Thick lines" in 
  val_control.add_change_listener (fun (x : bool) -> 
  if (x) then (paint.thickness <- 3) else (paint.thickness <- 1));
  widget  

(** Slider for thickness 
let slider_thickness () : widget = 
  let (widget, val_control) = slider 255 black in
  if (paint.thickness <- 3) 
*)
(** Slider for color values  *)
let slider_red () : widget = 
  let (widget, val_control) = slider 255 red in
  val_control.add_change_listener (fun (x : int) -> let g = paint.color.g in
                                              let b = paint.color.b in
                                              paint.color <- {r=x; g=g; b=b});

  widget

let slider_green () : widget = 
  let (widget, val_control) = slider 255 green in
  val_control.add_change_listener (fun (x : int) -> let r = paint.color.r in
                                              let b = paint.color.b in
                                              paint.color <- {r=r; g=x; b=b});

  widget

let slider_blue () : widget = 
  let (widget, val_control) = slider 255 blue in
  val_control.add_change_listener (fun (x : int) -> let r = paint.color.r in
                                              let g = paint.color.g in
                                              paint.color <- {r=r; g=g; b=x});
...
let mode_toolbar : widget = Widget.hlist [spacer; w_line; 
spacer; w_point; spacer; w_ellipse; spacer; checkbox_thickness (); 
spacer; w_undo]
...
let paint_widget =
   ...
   spacer; slider_red (); slider_green (); slider_blue ()]
